...
import { ChevronDown, Play, Sparkles, Languages } from 'lucide-react';
import { useEngineConnection } from '../../engine/useEngineConnection';
import { useDraft } from '../../storage/useHistory';
import { OnboardingBanner } from './OnboardingBanner';

interface ComposeViewProps {
  onStart: (article: string, options: { systemPrompt: string; voice: string }) => void;
}
...
export function ComposeView({ onStart }: ComposeViewProps) {
  ...
  const handleGenerate = () => {
    ...
    onStart(article, { systemPrompt, voice });
  };
  ...
  return (
    ...
    >
              ...
              <div className="flex gap-2">
                {(Object.keys(SAMPLES) as Array<keyof typeof SAMPLES>).map(key => (
                  <button
                    ...
                    className="px-3 py-1 rounded-full bg-black/5 hover:bg-black/10 text-[13px] text-text-secondary hover:text-text-primary transition-colors focus:outline-none focus:ring-2 focus:ring-accent-cyan"
      ...
      <div className="flex items-center gap-4 mb-8">
        <div className="relative">
          <select 
            ...
            className="appearance-none bg-black/5 border border-black/10 rounded-[var(--radius-small)] pl-8 pr-8 py-2 text-[13px] text-text-primary focus:outline-none focus:border-accent-purple"
          >
        ...
        <div className="relative">
          <select disabled className="appearance-none bg-black/5 border border-black/10 rounded-[var(--radius-small)] pl-8 pr-8 py-2 text-[13px] text-text-secondary opacity-70 cursor-not-allowed">
      ...
      {showAdvanced && (
        ...
        >
          ...
          <textarea
            ...
            className="w-full h-20 bg-background-secondary border border-black/10 rounded-[var(--radius-small)] p-3 text-[13px] font-mono text-text-primary focus:outline-none focus:border-accent-blue/50"
          />
        </motion.div>
      )}
      ...
      <div className="flex flex-col items-center gap-3 mt-auto">
        ...
        <span className="text-[13px] text-text-dim">~18 seconds · fully local · Cmd ↵</span>
      </div>
      </div>
    </motion.div>
  );
