...
export function TopBar({ currentView = 'studio' }: TopBarProps) {
  ...
  return (
    <>
      <header className="h-16 border-b border-black/5 flex items-center justify-between px-6 bg-background-primary/80 backdrop-blur-md sticky top-0 z-30 shrink-0">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-1.5 px-2 py-1 rounded bg-black/5 border border-black/5 text-[11px] font-mono text-text-dim cursor-pointer hover:text-text-secondary transition-colors" onClick={() => document.dispatchEvent(new KeyboardEvent('keydown', { metaKey: true, key: 'k' }))}>
            <span>⌘</span>
            <span>K</span>
          </div>
          <button
            onClick={() => document.dispatchEvent(new CustomEvent('navigate', { detail: currentView === 'studio' ? 'observability' : 'studio' }))}
            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs transition-colors ${currentView === 'observability' ? 'bg-black/10 border-black/10 text-text-primary' : 'bg-black/5 border-black/5 text-text-secondary hover:bg-black/10 hover:text-text-primary'}`}
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-black/5 border border-black/5 text-xs text-text-secondary hover:bg-black/10 hover:text-text-primary transition-colors"
          >
          ...
          <button 
            ...
            className="p-2 text-text-secondary hover:text-text-primary transition-colors rounded-full hover:bg-black/5"
          >
