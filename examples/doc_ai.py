...
Usage:
    ...
    python examples/doc_ai.py --images receipt.jpg invoice.png --prefer local
    python examples/doc_ai.py --images scan.pdf --detail high --prefer cloud
"""
...
def extract_document_data(
    ...
    print(f"\n🧾 Running Scenario S3: Document AI / Receipt & Invoice Extraction")
    print(f"📌 Input Image(s): {', '.join(images)}")
    print(f"⚙️  Routing Preference: {prefer} | Detail Tier: {detail}\n")
    ...
    if mock:
        print("ℹ️  Running in MOCK mode (synthetic extraction results & metrics)...")
        time.sleep(0.4)
        ...
        synthetic_data = {
            ...
            "items": [
                ...
                {
                    "description": "Avocado Toast w/ Poached Egg",
                    "quantity": 2,
                    "unit_price": 14.00,
                    "total_price": 28.00
                }
        ...
        return {
            ...
            "cost_usd": cost,
            "duration_sec": 0.42 if prefer == "local" else 0.85,
            "detail_tier": detail,
            "images": images
        }
    else:
        engine = MofaEngine(base_url=engine_url)
        print(f"⏳ Sending document image(s) to MoFA Engine understand API (prefer={prefer}, detail={detail})...")
        ...
        locality_str = getattr(res, "locality", "local" if prefer == "local" else "cloud")
        locality_tag = "\033[32mLocal (Free)\033[0m" if locality_str == "local" else "\033[38;2;249;115;22mCloud\033[0m"

        return {
            "json_data": parsed_json,
            "raw_text": raw_text,
            "model_used": getattr(res, "model_used", "qwen2.5-vl:7b" if prefer == "local" else "gpt-4o"),
            "locality": locality_tag,
            "cost_usd": getattr(res, "cost_usd", 0.0 if prefer == "local" else 0.00255),
...
def print_cost_comparison_table(current_prefer: str, current_detail: str):
    """Print colorized ANSI cost & billing detail comparison matrix."""
    header = "┌───────────────────────────────────┬───────────────┬───────────────┬──────────────────────────────────────────┐"
    title  = "│ Model / Configuration             │ Locality      │ Cost / Image  │ Resolution & Token Billing Tier          │"
    div    = "├───────────────────────────────────┼───────────────┼───────────────┼──────────────────────────────────────────┤"
    footer = "└───────────────────────────────────┴───────────────┴───────────────┴──────────────────────────────────────────┘"

    loc_local = "\033[32mLocal (Free)\033[0m"
    loc_cloud = "\033[38;2;249;115;22mCloud\033[0m"

    rows = [
        ("Local VLM (Qwen2.5-VL)", loc_local, "$0.000000", "Native local VLM (0 API bill, private)"),
        ("Cloud GPT-4o (detail='low')", loc_cloud, "~$0.000425", "Low res: 512x512 fixed tile (85 tokens)"),
        ("Cloud GPT-4o (detail='high')", loc_cloud, "~$0.002550", "High res: fine crop tiles (765+ tokens)"),
        ("Cloud GPT-4o (detail='auto')", loc_cloud, "Dynamic", "Auto selects low/high based on image dims"),
    ]

    print("\n📊 DOCUMENT AI COST & TIER COMPARISON MATRIX")
    print(header)
    print(title)
    print(div)
    ...
    print("\n💡 KEY OBSERVATIONS:")
    print("  • Local VLM: $0.00 financial cost, strict data privacy (PII stays local), no external API dependency.")
    print("  • Cloud GPT-4o Detail Tiers: 'low' processes at 512x512 (85 tokens), 'high' crops into multiple tiles for dense text.")
    print("  • MoFA Router: Automatically enforces prefer='local' unless cloud fallback is specified.\n")


def main():
    parser = argparse.ArgumentParser(description="Scenario S3: Document AI / Receipt & Invoice Extraction")
    parser.add_argument("--images", nargs="+", default=["receipt_sample.png"], help="Path(s) to document/receipt image files")
