...
if [ "$1" == "--status" ] || [ "$1" == "-s" ]; then
    ...
    if curl -s http://127.0.0.1:8420/health > /dev/null 2>&1; then
        echo -e "  • MoFA Engine Core (8420) : ${GREEN}🟢 Running (Healthy)${RESET}"
    else
        echo -e "  • MoFA Engine Core (8420) : ${YELLOW}⚪ Stopped${RESET}"
    fi

    # Check Frontend
    if curl -s http://localhost:3000 > /dev/null 2>&1; then
        echo -e "  • Web Studio Frontend (3000): ${GREEN}🟢 Running${RESET}"
    else
        echo -e "  • Web Studio Frontend (3000): ${YELLOW}⚪ Stopped${RESET}"
    fi

    # Check Ollama
    if curl -s http://127.0.0.1:11434/api/tags > /dev/null 2>&1; then
        echo -e "  • Ollama LLM Service (11434): ${GREEN}🟢 Running${RESET}"
    else
        echo -e "  • Ollama LLM Service (11434): ${YELLOW}⚪ Stopped${RESET}"
    fi
    exit 0
fi
...
if command -v cargo >/dev/null 2>&1; then
    echo -e "   ├─ Rust & Cargo : ${GREEN}✓ Found ($(cargo --version | cut -d' ' -f2))${RESET}"
else
    echo -e "   ├─ Rust & Cargo : ${YELLOW}⚠️ Not found. Please install Rust via https://rustup.rs${RESET}"
fi

if command -v python3 >/dev/null 2>&1; then
    echo -e "   ├─ Python 3     : ${GREEN}✓ Found ($(python3 --version))${RESET}"
else
    echo -e "   ├─ Python 3     : ${YELLOW}⚠️ Python 3 not found${RESET}"
fi

if command -v npm >/dev/null 2>&1; then
    echo -e "   ├─ Node / npm   : ${GREEN}✓ Found ($(node --version))${RESET}"
else
    echo -e "   ├─ Node / npm   : ${YELLOW}⚠️ Node.js / npm not found (needed for Web Studio)${RESET}"
fi

if command -v ffmpeg >/dev/null 2>&1; then
    echo -e "   └─ FFmpeg Media : ${GREEN}✓ Found${RESET}"
else
    echo -e "   └─ FFmpeg Media : ${YELLOW}⚠️ Optional (install via 'brew install ffmpeg' for video rendering)${RESET}"
fi
...
if ! curl -s http://127.0.0.1:8420/health > /dev/null 2>&1; then
    echo -e "   └─ Compiling & starting 'target/release/mofa-engine'..."
    cargo run --release -p mofa-engine > /tmp/mofa-engine.log 2>&1 &
    ...
    echo -e "   └─ ${GREEN}🟢 Engine Core is LIVE at http://127.0.0.1:8420${RESET}"
else
    echo -e "   └─ ${GREEN}🟢 Engine Core is already running on http://127.0.0.1:8420${RESET}"
fi
...
if [ -d "mofa-frontend" ] && command -v npm >/dev/null 2>&1; then
    ...
    if ! curl -s http://localhost:3000 > /dev/null 2>&1; then
        ...
        cd ..
        sleep 2
        echo -e "   └─ ${GREEN}🟢 Web Studio is LIVE at http://localhost:3000${RESET}"
    else
        echo -e "   └─ ${GREEN}🟢 Web Studio is already running at http://localhost:3000${RESET}"
    fi
fi
...
echo -e "  🌐 ${BOLD}Web Studio:${RESET}         http://localhost:3000"
echo -e "  🔌 ${BOLD}Engine API Gateway:${RESET} http://127.0.0.1:8420"
echo -e "  📊 ${BOLD}Metrics Export:${RESET}     http://127.0.0.1:8420/metrics"
